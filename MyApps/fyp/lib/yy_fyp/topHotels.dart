  ...
  Future<void> fetchTopHotels() async {
    ...
    try {
      final response = await http.get(
        Uri.parse(
          "$apiUrl?dest_id=$destId&dest_type=city&checkin_date=2026-06-01&checkout_date=2026-06-05&adults_number=2&room_number=1&order_by=popularity&locale=en-gb&filter_by_currency=MYR&units=metric",
        ),
        headers: {
          "X-RapidAPI-Key":
      ...
      if (response.statusCode == 200) {
        ...
        if (data["result"] is List) {
          setState(() {
            topHotels = List<Map<String, dynamic>>.from(
              data["result"]
                  .take(5)
                  .map(
                    (hotel) => {
        ...
        children: [
          ...
          TextButton(
            onPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(builder: (context) => seeAllHotels()),
              );
            },
            child: Text("See all"),
          ),
