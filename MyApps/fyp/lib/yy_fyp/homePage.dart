...
class _HomePageState extends State<homePage> {
  final String geoDbApiKey = "99d0568adcmsh612a2ca3d0334f9p15fdf5jsndc687769b285";
...
Future<String> fetchExperienceImage(String xid) async {
  final String apiKey = "5ae2e3f221c38a28845f05b61e71a6719cee59a2f24d1b01fe74b4ef";
  final url = Uri.parse("https://api.opentripmap.com/0.1/en/places/xid/$xid?apikey=$apiKey");
...
// Fetch city details from GeoDB Cities API (Hardcoded Cities)
  Future<Map<String, dynamic>> fetchCityDetails(String city, String countryCode) async {
    final url = Uri.parse("https://wft-geo-db.p.rapidapi.com/v1/geo/cities?namePrefix=$city&countryIds=$countryCode");
        ...
        onTap: () {
          if (id != null) {
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => experienceDetailsScreen(experienceName: title),
              ),
            );
          }
        },
...
Widget _buildDestinationList(List<dynamic> destinations, BuildContext context) {
    ...
    itemBuilder: (context, index) {
      ...
      return Card(
        ...
        child: ListTile(
          ...
          subtitle: Text("Country: ${destination["country"]}"), // Display country
          onTap: () {
            Navigator.push(
              context,
                MaterialPageRoute(
                  builder: (context) => destinationDetailsScreen(cityName: destination["city"]),
              ),
            );
          },
        ),
