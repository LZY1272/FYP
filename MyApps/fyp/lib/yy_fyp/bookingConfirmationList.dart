...
class bookingConfirmationList extends StatefulWidget {
  ...
  @override
  _bookingConfirmationListState createState() => _bookingConfirmationListState();
}
    ...
    if (paidResponse.statusCode == 200) {
      ...
      setState(() {
        bookings = bookings.where((booking) {
          String bookingId = "${booking['hotelName']}_${booking['dateRange']}";
  ...
  Future<void> fetchBookings() async {
    try {
      ...
      final response = await http.get(
        Uri.parse('http://10.0.2.2:3000/bookings?userId=${Uri.encodeComponent(userId!)}'),
      );
